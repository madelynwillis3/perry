...
get_top_correlated <- function(soil_var, corr_matrix, n = 3) {
  top_vars <- names(sort(abs(corr_matrix[soil_var, ]), decreasing = TRUE)[1:n])
  return(top_vars)
}
```
...
for (var in soil_vars) {
  # Subset soil points
  soil_sub <- soil_points[, c("geometry", var)]
  
  # Convert to spatial object
  soil_sp <- as(soil_sub, "Spatial")
  
  # Fit variogram and kriging model
  vgm_model <- autofitVariogram(as.formula(paste(var, "~ 1")), soil_sp)
  krige_result <- krige(as.formula(paste(var, "~ 1")), soil_sp, topo_rasters, model = vgm_model$var_model)
  
  # Convert kriged data to raster
  r_kriged <- rast(krige_result)
  names(r_kriged) <- var
  
  # Store in list
  kriged_rasters[[var]] <- r_kriged
}
...
```{r DSM method}
# -------------------------------
# RANDOM FOREST FOR SOIL VARIABLES
# -------------------------------

# Create empty list to store predicted rasters
rf_rasters <- list()

# Loop through soil variables
for (var in soil_vars) {
  # Get top correlated topo rasters
  top_rasters <- get_top_correlated(var, corr_matrix, n = 3)
  
  # Extract raster values at soil sample points
  extracted_data <- extract(topo_stack[[top_rasters]], soil_data)
  
  # Combine with soil values
  model_data <- cbind(soil_data[var], extracted_data)
  model_data <- na.omit(model_data)  # Remove NAs
  
  # Train Random Forest model
  rf_model <- randomForest(as.formula(paste(var, "~ .")), data = model_data, ntree = 500)
  
  # Predict across the study area
  pred_raster <- predict(topo_stack[[top_rasters]], rf_model)
  names(pred_raster) <- var
  
  # Store in list
  rf_rasters[[var]] <- pred_raster
}

# Stack all RF-predicted rasters
rf_stack <- do.call(c, rf_rasters)

# Save RF raster stack
writeRaster(rf_stack, "rf_soil_stack.tif", overwrite = TRUE)
```
#Classification
```{r Classification}
